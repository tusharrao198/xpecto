...
module.exports = {
	...
	findWorkshop: async function (req) {
		...
		const workshop = await workshopTable
			.findOne({ name: params.workshop })
			.lean();
		return workshop;
	},
	...
	numberofReg_referCode: async function () {
		...
		// console.log("refer = ", refercodedata);
		...
		for (let i = 0; i < refercodedata.length; i++) {
			...
			for (let j = 0; j < regdata.length; j++) {
				if (
					...
					if (
						regdata[j] &&
						refercodedata[i] &&
						refercodedata[i].codes &&
						regdata[j].referralCode.toUpperCase() ===
							refercodedata[i].codes.toUpperCase()
					) {
						regcnt += 1;
					}
				}
			...
			let addd = {
				...
				referralCode: refercodedata[i].codes,
			};
			referdata.push(addd);
		}
